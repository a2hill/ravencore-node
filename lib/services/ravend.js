...
/**
 * Provides a friendly event driven API to ravend in Node.js. Manages starting and
 * stopping ravend as a child process for application support, as well as connecting
 * to multiple ravend processes for server infrastructure. Results are cached in an
...
Ravencoin.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ravend/' + name, 'total:', this.subscriptions[name].length);
};
...
Ravencoin.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ravend/' + name, 'total:', this.subscriptions[name].length);
};
...
Ravencoin.prototype.subscribeBalance = function(emitter, addresses) {
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (ravencore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'ravend/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
Ravencoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'ravend/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
Ravencoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ravend/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Ravencoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ravend/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Ravencoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in ravend config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in ravend config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in ravend config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './raven.conf');

  log.info('Using ravencoin config file:', configPath);
...
Ravencoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of ravencore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of ravencore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of ravencore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to ravend from ravencore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from ravend. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from ravend. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in raven.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that ravend is undergoing a reindex. ' +
             ...
             'of ravencore-node services will start.');
    node._reindex = true;
  }
};
...
Ravencoin.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === ravencore.Networks.testnet
...
Ravencoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === ravencore.Networks.testnet) {
    networkPath = 'testnet6/raven.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/raven.conf';
    }
  }
  return networkPath;
};
...
Ravencoin.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('ravend/hashblock', message.toString('hex'));
    }
  }
    ...
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('ravend/addressbalance', {
                    address: address,
                    txid: txid,
...
Ravencoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('ravend/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Ravencoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = ravencore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('ravend/rawtransaction', message.toString('hex'));
    }
...
Ravencoin.prototype._stopSpawnedRavencoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/ravend.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned ravencoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean ravencoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Ravencoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedRavencoin(function(err) {
    ...
    log.info('Starting ravencoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        ...
        log.warn('Restarting ravencoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn ravend.'));
      }
...
Ravencoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to ravend.'));
    }
...
Ravencoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = ravencore.Script.fromAddress(delta.address);
    var result = {
...
Ravencoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in ravencore is most recent to less recent
 ...
 * @param {Object} txid - A ravencoin transaction id
 * @param {Function} callback
 */
...
Ravencoin.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof ravencore.Address) {
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = ravencore.Block.fromString(response.result);
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('ravend spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('ravend process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.listAssets = function(asset, verbose, size, skip, callback) {
  var self = this;
